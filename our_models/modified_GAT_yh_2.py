...
class modified_GAT(MessagePassing):
    def __init__(
        ...
        self.lin_edge = Linear(edge_dim, heads * out_channels, bias=False,
                               weight_initializer='glorot')
        self.att_edge = Linear(edge_dim, heads * kq_dim, bias=False,
                               weight_initializer='glorot')
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        edge_query = self.att_edge(edge_attr).view(-1, H, self.kq_dim)
        ...
        out = self.propagate(edge_index, x=x_value, query=query, key=key, edge_attr=edge_attr,
                             edge_value=edge_value, edge_query=edge_query, size=size)
    ...
    def message(self, x_j: Tensor, query_j: Tensor, query_i: Tensor, key_j: Tensor,
                key_i: Tensor, edge_attr: OptTensor, edge_value: OptTensor, edge_query: OptTensor,
                index: Tensor, ptr: OptTensor, size_i: Optional[int]) -> Tensor:
        query = F.leaky_relu(query_i + query_j + edge_query, self.negative_slope)
        ...
        #     edge_attr = self.lin_edge(edge_attr[:, 1:])
        #     edge_attr = edge_attr.view(-1, self.heads, self.kq_dim)
        #     alpha_edge = (edge_attr * self.att_edge).sum(dim=-1)
        #     score = score + alpha_edge

        alpha = softmax(score, index, ptr, size_i)
        self._alpha = alpha  # Save for later use.
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)

        return (x_j + edge_value) * alpha.unsqueeze(-1)
