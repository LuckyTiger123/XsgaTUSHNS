...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.temporal_encoders = torch.nn.ModuleList()
        ...
        self.edge_mlps.append(torch.nn.Linear(data.edge_attr.size(1) + hidden_size - 1, hidden_size))
        self.temporal_encoders.append(TimeEncoder(hidden_size))
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        for i in range(self.layer_num):
            t_emb = self.temporal_encoders[i](t)
            edge_msg = self.edge_mlps[i](torch.cat((edge_feat, t_emb), dim=1))
            x = self.bns[i](x)
