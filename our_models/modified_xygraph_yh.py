...
from our_models import load_data as ld
...
def read_xygraphp1(folder):
    ...
    x = torch.tensor(items[0]['x'], dtype=torch.float).contiguous()
    y = torch.tensor(items[0]['y'], dtype=torch.int64)
    edge_index = torch.tensor(items[0]['edge_index'].transpose(), dtype=torch.int64).contiguous()
